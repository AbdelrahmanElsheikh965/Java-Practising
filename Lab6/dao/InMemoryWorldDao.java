package labfiles.lab.dao;

import labfiles.lab.domain.City;
import labfiles.lab.domain.Country;
...
public class InMemoryWorldDao implements WorldDao {
	...
	private InMemoryWorldDao() {
		...
		for (City city : cities.values()) {
			// get country object based on country code of each city
	...
	public static void main(String args[]) {
		...
		/* First One */
		Map<String, String> highestPopulatedCitiesByCountry = countries.values().stream() // Stream of Country objects
				.collect(Collectors.toMap(
						// key
						Country::getName,
						// value
						country -> country.getCities().stream() // Stream<List<City>> for each country
								.max(Comparator.comparingInt(City::getPopulation)) // Find max population city
								.map(City::getName) // Map City to its name
								.orElse("No city found") // If no city found, set as "No city found"
				));
            System.out.println("highest populated city of each country" + highestPopulatedCitiesByCountry);

		/* Second One */
		Map<String, String> highestPopulatedCityOfEachContinent = countries.values().stream()
				.collect(Collectors.groupingBy(Country::getContinent, // Grouping by continent
						Collectors.collectingAndThen(Collectors.mapping(
								country -> country.getCities().stream().mapToInt(City::getPopulation).max().orElse(0),
								Collectors.reducing(Integer::max) // Calculate maximum population for each continent
						), maxPopulation -> maxPopulation.map(max ->

						// Map maxPopulation to country object that contains the highest city
						// population.
						countries.values().stream()
								// Here we are getting the country object.
								.filter(country -> country.getCities().stream()
										.anyMatch(city -> city.getPopulation() == max))
								.findFirst()

								.map(country -> country.getCities().stream().filter(city -> city.getPopulation() == max)
										// Last step : we got the city name with max population of each continent.
										.map(City::getName).findFirst().orElse("No city found"))

								.orElse("No city found")).orElse("No city found"))));
		System.out.println("Highest Populated City Of Each Continent: " + highestPopulatedCityOfEachContinent);

		
		/* Third One */
		Optional<City> highestPopulatedCapital = countries.values().stream()
				// Flatten all cities from countries
				// Get collection of all cities in the world.
				.flatMap(country -> country.getCities().stream())

				.filter(city -> {
					// Get country code of each city and check if it's a capital city or not?
					Country country = countries.get(city.getCountryCode()); // Get corresponding country

					// Return the new stream containing all the capitals only.
					return city.getId() == country.getCapital(); // Filter for cities matching the country's capital ID

				})
				// Print the returned city object as toString implemented inCity class.
				.max(Comparator.comparingInt(City::getPopulation));
		System.out.println("Highest Populated Capital: " + highestPopulatedCapital);
